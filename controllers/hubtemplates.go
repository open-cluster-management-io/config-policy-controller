...
func resolveHubTemplates(ctx context.Context, r hubResolver, policyCopy client.Object) error {
	...
	if err != nil {
		if cacheGetErr != nil && !k8serrors.IsNotFound(cacheGetErr) {
			return err // TOBEDONE: consider if we should / if it's possible to requeue in this case
		}

		if !k8serrors.IsNotFound(cacheGetErr) { // check the generation to actually verify if it's up-to-date
			inputGeneration := policyCopy.GetGeneration()

			jsonErr := json.Unmarshal(cachedResultObject.Data["policy.json"], policyCopy)
			if jsonErr != nil {
				return err // TOBEDONE: specially log this?
			}
		...
		log.Error(err, "some other failure to resolve template")
	...
	if jsonErr != nil {
		// TOBEDONE: specially log this?
		log.Error(jsonErr, "unable to unmarshal resolved JSON into a policy")
	...
	if k8serrors.IsNotFound(cacheGetErr) {
		...
		// TOBEDONE: is there anything else we can do here? Seems like we have to retry.
